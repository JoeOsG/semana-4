...
function funcFilterByTag(entry) {
    return _.includes(entry.tags, "Desarrollo");
}
const dataFilteredByTag = _.filter(entries, funcFilterByTag);

const dataGrouppedByAutor = _.groupBy(dataFilteredByTag, "autor");

function funcMapByTitle(data) {
    return _.map(data, "titulo")
}
const a = _.mapValues(dataGrouppedByAutor, funcMapByTitle)
console.log(a);
